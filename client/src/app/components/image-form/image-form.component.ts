...
export class ImageFormComponent {
  ...
  validateForm() {
    if (this.checked_2_factor === true && this.checked_4_factor === true) {
      ...
      this.checked_2_factor === false &&
      this.checked_4_factor === false
    ) {
      this.scale_factor = 0;
      return window.alert('You need to select a value');
    }
    this.checked_2_factor === true
      ? (this.scale_factor = 2)
      : (this.scale_factor = 4);
    this.onSubmit();
  }
  ...
  onSubmit() {
    ...
    this.http
      .post(
        `http://localhost:8000/scale?scaling=${this.scale_factor}`,
        formData
      )
      .subscribe((response: any) => {
        if (response) {
          ...
          this.http
            .get<Blob>('http://localhost:8000/image/' + path, {
